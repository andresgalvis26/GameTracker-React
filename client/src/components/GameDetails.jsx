...
const GameDetails = ({ game, onEdit, onClose }) => {
    ...
    return <div className="bg-gray-900">
        <div className="relative overflow-hidden border-b border-gray-800 bg-gradient-to-br from-blue-950 via-gray-900 to-gray-950 p-5 sm:p-8">
            ...
            <div className="relative grid gap-7 md:grid-cols-[220px_1fr] md:items-center">
                ...
                <div className="min-w-0">
                    <div className="mb-4 flex flex-wrap gap-2"><span className={`rounded-full border px-3 py-1 text-xs font-bold ${statusClass}`}>{game.status}</span><span className="rounded-full border border-gray-600 bg-gray-800/80 px-3 py-1 text-xs font-semibold text-gray-300">{game.platform}</span>{game.pcStore && <span className="rounded-full border border-gray-600 bg-gray-800/80 px-3 py-1 text-xs text-gray-300">🛒 {game.pcStore}</span>}</div>
                    ...
                    <div className="mt-6 flex flex-wrap items-end gap-6">
                        ...
                        <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-gray-500">{game.status === 'Completado' ? 'Completado' : 'Objetivo'}</p><p className={`mt-1 text-2xl font-bold ${targetYear ? 'text-white' : 'text-gray-500'}`}>{targetYear || 'Sin definir'}</p></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-6 p-5 sm:p-8">
            ...
            <section><SectionTitle eyebrow="Resumen" title="Información del juego" /><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Info label="Estado" value={game.status} /><Info label="Plataforma" value={game.platform} /><Info label="Rating" value={hasRating ? `${Number(game.rating).toFixed(1)} / 10` : 'Sin puntuar'} /><Info label={game.status === 'Completado' ? 'Año completado' : 'Año objetivo'} value={targetYear ? String(targetYear) : 'Sin definir'} /></div></section>
