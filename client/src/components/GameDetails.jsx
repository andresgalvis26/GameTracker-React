        ...
        <div className="relative overflow-hidden border-b border-gray-800 bg-gradient-to-br from-blue-950 via-gray-900 to-gray-950 p-5 sm:p-8">
            ...
            <div className="relative grid gap-7 md:grid-cols-[220px_1fr] md:items-center">
                ...
                <div className="min-w-0">
                    <div className="mb-4 flex flex-wrap gap-2"><span className={`rounded-full border px-3 py-1 text-xs font-bold ${statusClass}`}>{game.status}</span>{game.wishlist && <span className="rounded-full border border-pink-400/30 bg-pink-400/10 px-3 py-1 text-xs font-bold text-pink-200">🎯 En lista de deseos</span>}{game.isOnline && <span className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 text-xs font-bold text-always-cyan">🌐 Online · No cuenta para progreso</span>}<span className="rounded-full border border-gray-600 bg-gray-800/80 px-3 py-1 text-xs font-semibold text-always-white">{game.platform}</span>{game.pcStore && <span className="rounded-full border border-gray-600 bg-gray-800/80 px-3 py-1 text-xs text-always-white">🛒 {game.pcStore}</span>}{Number(game.hoursPlayed) > 0 && <span className="rounded-full border border-gray-600 bg-gray-800/80 px-3 py-1 text-xs font-semibold text-always-white">⏱️ {Number(game.hoursPlayed)} h</span>}</div>
                    <h1 className="max-w-2xl text-3xl font-black tracking-tight text-always-white sm:text-4xl">{game.title}</h1>
