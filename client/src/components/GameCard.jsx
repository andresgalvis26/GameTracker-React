...
const GameCard = ({ game, onDelete, onClick, onEdit }) => {
    ...
    <button type="button" onClick={() => onClick(game)} className="block w-full text-left focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-400">
        ...
        <div className="p-4"><div className="flex items-start justify-between gap-3"><h2 id={`game-${game.id}`} className="line-clamp-2 min-h-12 font-bold text-lg leading-6 text-white">{game.title}</h2><span className="shrink-0 rounded-md bg-gray-700 px-2 py-1 text-xs font-semibold text-gray-300">{game.platform}</span></div><div className="mt-4 grid grid-cols-2 gap-2 border-t border-gray-700/70 pt-3 text-xs"><div><span className="block text-gray-500">{game.status === 'Completado' ? 'Completado' : 'Año objetivo'}</span><span className={`mt-1 block font-bold ${targetYear ? 'text-blue-300' : 'text-gray-500'}`}>{targetYear || 'Sin definir'}</span></div><div className="text-right"><span className="block text-gray-500">Añadido</span><time className="mt-1 block text-gray-400" dateTime={game.createdAt}>{new Date(game.createdAt).toLocaleDateString('es-ES')}</time></div></div></div>
    </button>
