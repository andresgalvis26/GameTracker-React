    ...
    return <form onSubmit={validateAndSubmit} className="bg-gray-900" noValidate>
        <div className="grid gap-0 lg:grid-cols-[280px_1fr]">
            ...
            <div>
                ...
                <section className="p-5 sm:p-7"><SectionKicker>Tu seguimiento</SectionKicker><div className="mt-4 grid gap-5 sm:grid-cols-2"><div><div className="flex items-end justify-between"><label htmlFor="rating" className="text-sm font-medium text-gray-300">Puntuación</label><span className="text-lg font-black text-yellow-300">{form.rating === '' ? 'Sin puntuar' : `${Number(form.rating).toFixed(1)} / 10`}</span></div><input id="rating" type="range" min="0" max="10" step="0.1" value={form.rating === '' ? 0 : form.rating} onChange={(event) => update({ rating: event.target.value })} className="mt-4 w-full accent-yellow-400" aria-label="Puntuación de 0 a 10" />{errors.rating && <ErrorMessage>{errors.rating}</ErrorMessage>}</div><label htmlFor="target-year" className="block text-sm font-medium text-gray-300">{completed ? 'Año de completado' : 'Año objetivo'}<select id="target-year" value={form.targetYear} onChange={(event) => update({ targetYear: event.target.value })} className="mt-2 w-full rounded-lg border border-gray-700 bg-gray-800 p-3 text-white"><option value="">Sin especificar</option>{years().map((year) => <option key={year} value={year}>{year}</option>)}</select></label><label htmlFor="hours-played" className="block text-sm font-medium text-gray-300">Horas jugadas <span className="font-normal text-gray-500">(opcional)</span><input id="hours-played" type="number" min="0" step="0.5" inputMode="decimal" value={form.hoursPlayed} onChange={(event) => update({ hoursPlayed: event.target.value })} placeholder="Ej. 12.5" className={`mt-2 w-full rounded-lg border bg-gray-800 p-3 text-white ${errors.hoursPlayed ? 'border-red-400' : 'border-gray-700'}`} />{errors.hoursPlayed && <ErrorMessage>{errors.hoursPlayed}</ErrorMessage>}</label></div><div><p className="mb-2 text-sm font-medium text-gray-300">Géneros</p><div className="flex flex-wrap gap-2">{GENRES.map((genre) => { const active = form.genres.includes(genre); return <button key={genre} type="button" onClick={() => update({ genres: active ? form.genres.filter((item) => item !== genre) : [...form.genres, genre] })} aria-pressed={active} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${active ? 'border-blue-400 bg-blue-500/20 text-blue-200' : 'border-gray-700 bg-gray-800 text-gray-400 hover:border-gray-500 hover:text-white'}`}>{genre}</button>; })}</div></div><div className="grid gap-3 sm:grid-cols-2"><Check label="🎯 Lista de deseos" hint="Lo quiero jugar más adelante" checked={Boolean(form.wishlist)} onChange={(value) => update({ wishlist: value })} />{completed && <><Check label="Lo volvería a jugar" hint="Vale la pena repetirlo" checked={form.replayable} onChange={(value) => update({ replayable: value })} /><Check label="Platinado / 100%" hint="Todos los logros conseguidos" checked={form.platinated} onChange={(value) => update({ platinated: value })} /></>}</div></section>
            </div>
        </div>
